...
def calculate(days, levels, bonus_list):
    ...
    for i in range(1, days + 1):
        ...
        profitable_to_skip_the_day = sum(bonus_list[levels - level:]) < sum(
            bonus_list[:levels - level - 1]) or sum(bonus_list[level - 1:]) < sum(bonus_list[:level - 1])

        if level >= levels:
            bonuses += bonus_list[level - 1]
            if IS_DEBUG:
                print('Level reached maximum',
                      level, 'bonus', bonus_list[level - 1])
            level = 1
        elif few_moves_left:
            bonuses += bonus_list[level - 1]
            if IS_DEBUG:
                print('Days are few. Press the button', level,
                      'bonus', bonus_list[level - 1])
            level += 1
        elif profitable_to_take_a_bonus:
            bonuses += bonus_list[level - 1]
            if IS_DEBUG:
                print('Press the button', level,
                      'bonus', bonus_list[level - 1])
            level += 1
        elif profitable_to_skip_the_day:
            if IS_DEBUG:
                print('Skip the turn', level, 'bonus', bonus_list[level - 1])
            level = 1
